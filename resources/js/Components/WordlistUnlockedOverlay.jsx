...
import { X, Share2, Facebook, MessageCircle, Send, Instagram, ArrowRight, Unlock, Image as ImageIcon } from "lucide-react";
...
export default function WordlistUnlockedOverlay({ listName, onDismiss }) {
    ...
    const shareText = t("achievements.overlay.unlocked_share_text", { name: listName });
    ...
    const handleShareImage = async () => {
        ...
        try {
            ...
            const file = new File([blob], `unlocked-${listName.toLowerCase().replace(/\s+/g, '-')}.png`, { type: 'image/png' });
            ...
            if (navigator.canShare && navigator.canShare({ files: [file] })) {
                ...
                link.download = `unlocked-${listName.toLowerCase().replace(/\s+/g, '-')}.png`;
                link.href = dataUrl;
                link.click();
        ...
        <div className={`fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md transition-opacity duration-300 ${isClosing ? "opacity-0" : "opacity-100"} animate-in fade-in`}>
            ...
            >
                ...
                <div className="pt-10 pb-6 px-8 text-center">
                    <span className="text-[10px] font-bold tracking-[0.2em] text-blue-500 uppercase mb-2 block">
                ...
                <div className="relative flex justify-center items-center py-4">
                    ...
                    <div className="relative z-10 p-6 bg-blue-100 dark:bg-blue-900/30 rounded-full">
                        <Unlock className="w-20 h-20 text-blue-600 dark:text-blue-400" />
                    </div>
                </div>
